...
function ksr_request_route()
    KSR.info("===== Starting Request route\n");
    route_auth()
    route_registrar()
    route_natmanage()
    route_canceling()
...
function route_auth()
    if KSR.is_method("REGISTER") then
        if not KSR.auth.pv_auth_check(KSR.pv.get("$fd"), "subscriber", "1") then
            KSR.xlog.xinfo("L_INFO", "REGISTER from " .. KSR.pv.get("$si") .. "\n")
...
function route_registrar()
    if not KSR.is_method("REGISTER") then
        KSR.xlog.xinfo("Non-REGISTER request received: " .. KSR.pv.get("$rm"))
        return 1
    end
...
function route_natmanage()
    local avp_received = KSR.pv.get("$avp(RECEIVED)") or "nil"
    KSR.force_rport();
...
function route_withindlg()
    ...
    if KSR.rr.loose_route() then
        return 1  -- If use loose_route return 1
    end

    -- if use loose_route next route go to route DLURI
    if KSR.rr.loose_route_mode("1") then
        KSR.route("DLGURI")  
    end

    if KSR.is_method_in("ACK") then
